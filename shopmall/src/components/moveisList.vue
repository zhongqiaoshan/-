            ...
            >
              ...
              <div class="comingSoonFilm-info">
                ...
                <div class="comingSoonFilm-grade">
                  <span class="gradees" v-if="item.grade">观众评分</span>
                  <span class="yeallow">{{ item.grade }}</span>
                </div>
                <div class="comingSoonFilm-actors over-hiden">
                  <span class="label">主演：{{ moviesList[index].name }}</span>
                </div>
...
export default {
  ...
  methods: {
    ...
    onRefresh() {
      ...
      if (this.total == this.moviesLists.length) {
        ...
        var cityId = window.localStorage.getItem("curCityId")
          ? window.localStorage.getItem("curCityId")
          : 110100;
        console.log(cityId);
        ...
        axios
          .get(
            `https://m.maizuo.com/gateway?cityId=${cityId}&pageNum=${this
              .pageNum + 1}&pageSize=${this.pageSize}&type=1&k=1016945`,
            {
              headers: {
                "X-Client-Info":
  ...
  mounted() {
   ...
   var cityId = window.localStorage.getItem("curCityId")?  window.localStorage.getItem("curCityId") : 110100;
    ...
    axios
      .get(
        `https://m.maizuo.com/gateway?cityId=${ cityId }&pageNum=1&pageSize=10&type=1&k=9152149`,
        {
          headers: {
            "X-Client-Info":
              '{"a":"3000","ch":"1002","v":"5.0.4","e":"15605021417275674601907"}',
